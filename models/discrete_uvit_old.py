...
class UViT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.num_classes = num_classes + 1  # inlcude MASK
    ...
    def _forward_with_cfg(
        ...
        if cfg_scale == 0.0:
            uncond_logits = self._forward(
                x.clone(), t.clone(), y.clone(), cond_drop_prob=1.0
            )
            return uncond_logits
        elif cfg_scale == 1.0:
            cond_logits = self._forward(
                x.clone(), t.clone(), y.clone(), cond_drop_prob=0.0
            )
            return cond_logits
    ...
    def forward_without_cfg(
        ...
        return self._forward_compile_wrapper(x, t, y, cond_drop_prob=0.0)
    ...
    def _forward(
        ...
        time_token = time_token.unsqueeze(dim=1).to(self.dtype)
        x = torch.cat((time_token, x), dim=1)
        if self.num_classes > 0:
            label_emb = self.label_emb(
                y, train=self.training, cond_drop_prob=cond_drop_prob
            )
            label_emb = label_emb.unsqueeze(dim=1).to(self.dtype)
            x = torch.cat((label_emb, x), dim=1)
        elif self.num_classes == -666:
            text_embedding = self.context_embed(
                y.to(self.dtype), train=self.training, cond_drop_prob=cond_drop_prob
            )
