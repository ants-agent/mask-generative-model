...
class Latte(nn.Module):
    ...
    def __init__(
        ...
        print(f"time_cond: {time_cond},use_checkpoint: {use_checkpoint}")
        ...
        elif self.extras == 78:  # timestep + text_embedding
    ...
    def forward_with_cfg(
        ...
        cond_logits = self._forward(
            ...
            ),  # y can be None for unconditional generation
            text_embedding,
            use_fp16=use_fp16,
            cond_drop_prob=0.0,
        )
    ...
    def _forward(
        ...
        if not self.time_cond:
            t = torch.zeros(bs, dtype=t.dtype, device=x.device)
        ...
        if self.extras == 2:
            y = self.y_embedder(y, self.training, cond_drop_prob=cond_drop_prob)
            ...
            text_embedding = self.text_embedding_projection(
                text_embedding.reshape(bs, -1), cond_drop_prob=cond_drop_prob
            )
            text_embedding_spatial = repeat(
                text_embedding, "n d -> (n c) d", c=self.temp_embed.shape[1]
            )
            text_embedding_temp = repeat(
                text_embedding, "n d -> (n c) d", c=self.pos_embed.shape[1]
            )
        ...
        for i in range(0, len(self.blocks), 2):
            ...
            x = rearrange(x, "(b f) t d -> (b t) f d", b=bs)
            # Add Time Embedding
            if i == 0:
                x = x + self.temp_embed
            ...
            x = temp_block(x, c)
            x = rearrange(x, "(b t) f d -> (b f) t d", b=bs)
