...
def _eval_by_npz(ref_batch, sample_batch):
    ...
    return dict(IS=IS, FID=FID, sFID=sFID, prec=prec, recall=recall)


@click.command()
@click.option("--ref_dir", help="path to reference directory")
@click.option("--sample_dir", help="path to sample directory")
def eval_dir(ref_dir, sample_dir, debug=True):
    """
    python utils/openai_eval.py  --ref_dir samples/sample_vq_v2_imagenet256_cond_indices_uvit2dv2_h2_bs40fid50000cfg1.6smt1.0_10462892/gts --sample_dir samples/sample_vq_v2_imagenet256_cond_indices_uvit2dv2_h2_bs40fid50000cfg1.6smt1.0_10462892/samples
    """

    def load_images_from_dir(directory, num_samples=None):
        image_files = [
        ...
        if debug:
            ...
            print("only use 100 images for debug")

        images = []
        for img_file in tqdm(image_files, desc=f"Loading images from {directory}"):
    ...
    # Load images
    sample_images = load_images_from_dir(sample_dir)
    real_images = load_images_from_dir(ref_dir)

    # Ensure same number of images in both sets
    min_samples = min(len(sample_images), len(real_images))
    sample_images = sample_images[:min_samples]
    real_images = real_images[:min_samples]
    print(
        f"sample_images shape: {sample_images.shape}, real_images shape: {real_images.shape}"
    )

    # Save as npz files
    sample_npz = "temp_sample.npz"
    real_npz = "temp_real.npz"
    np.savez(sample_npz, sample_images)
    np.savez(real_npz, real_images)

    # Evaluate
    eval_dict = _eval_by_npz(ref_batch=real_npz, sample_batch=sample_npz)
