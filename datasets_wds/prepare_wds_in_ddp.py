...
wds_target_dir = (
    f"./data/imagenet{image_size}_shuffled_raw_wds_train_realflip1_indices_v5"
)
...
if is_debug:
    wds_target_dir = wds_target_dir + "_debug"
    batch_size = 2
...
def SD_VQ_tokenizer():
    ...
    _tokenizer = accelerator.prepare(_tokenizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--path", type=str, default=DATASET_ROOT, help="path to dataset"
    )
    parser.add_argument(
        "-p2",
        "--wds_target_dir",
    ...
    if accelerator.is_main_process:
        writer = wds.ShardWriter(
            ...
            maxcount=1e6,
            maxsize=opt.max_size * 1e9 * size_per_wds,
    ...
    for _iddd, (img_pairs, cls_id, cls_name) in tqdm(
        ...
        if is_debug:
            for _id, (_indices, _cls_id) in enumerate(zip(indices, cls_id)):
                print(
                    f"rank: {accelerator.process_index}, _id: {_id}, indices[3,2]: {_indices[3,2].item()}, cls_id: {_cls_id.item()}"
                )
                accelerator.wait_for_everyone()
        indices = accelerator.gather(indices.contiguous().to(device))
        cls_id = accelerator.gather(cls_id.contiguous().to(device))
        ...
        if accelerator.is_main_process:
            ...
            for _indices, _cls_id in zip(indices, cls_id):
                ...
                wds_dict["indices.npy"] = _indices.cpu().numpy().astype(np.int32)
                wds_dict["cls_id.cls"] = int(_cls_id)
                file_index += 1
    ...
    # CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=4,5,6,7 accelerate launch  --mixed_precision no  --num_processes 4 --num_machines 1 --multi_gpu --main_process_ip 127.0.0.1 --main_process_port 8878  datasets_wds/wds_prepare_in_ddp.py
