...
def _main(cfg):
    ...
    accelerator = accelerate.Accelerator(cfg.mixed_precision, gradient_accumulation_steps=cfg.accum)
    if cfg.accum > 1:
    ...
    if cfg.ckpt is not None:  # before accelerate.wrap()
        ...
        if accelerator.state.num_processes == 1:
            ...
            state_dict["model"]["shared_embed_table_x.weight"] = state_dict[
                "model"
            ].pop("x_embed.weight")
            state_dict["model"].pop("final_layer.weight")
            state_dict["model"].pop("final_layer.bias")
            state_dict["model"] = {
                "attend_model." + k: v
                for k, v in state_dict["model"].items()
                if k.startswith("in_blocks")
                or k.startswith("mid_block")
                or k.startswith("out_blocks")
                or k == "norm.weight"
                or k == "norm.bias"
            }
        else:
            state_dict["model"]["module.shared_embed_table_x.weight"] = state_dict[
                "model"
            ].pop("module.x_embed.weight")
            state_dict["model"]["module.final_layer.weight"] = state_dict["model"].pop(
                "module.final_layer.weight"
            )
            state_dict["model"]["module.final_layer.bias"] = state_dict["model"].pop(
                "module.final_layer.bias"
            )
            state_dict["model"] = {
                k.replace("module.", "module.attend_model."): v
                for k, v in state_dict["model"].items()
                if k.startswith("module.in_blocks")
                or k.startswith("module.mid_block")
                or k.startswith("module.out_blocks")
                or k == "module.norm.weight"
                or k == "module.norm.bias"
            }
        ...
        try:
            ...
            if accelerator.is_main_process:
                logging.info("load_state_dict error", e)
    ...
    @torch.no_grad()
    def sample_cond2img(bs, cfg, _sample_size=None, sample_mode="mask2img"):
        model.eval()
        ...
        #############
        sample_kwargs["sample_mode"] = sample_mode
        #############
        try:
            with torch.no_grad():
    ...
    @torch.no_grad()
    def sample_img2cond(bs, cfg, _sample_size=None, sample_mode="img2mask"):
        model.eval()
        if _sample_size is None:
            _sample_size = vq_get_sample_size(bs, cfg)
        else:
        ...
        if "imagenet" in cfg.data.name and cfg.data.num_classes > 0:
            ...
            x = encode_fn(x)
            sample_kwargs["x"] = x[:bs].clone()
            sample_kwargs["y"] = y[:bs].clone()
            assert len(sample_kwargs["x"]) == bs
            assert len(sample_kwargs["y"]) == bs
        else:
            sample_kwargs["y"] = None

        #############
        if cfg.use_cfg:
            assert "y" in sample_kwargs, "y must be provided for cfg sampling"
            print_rank_0(f"using cfg for sampling, cfg_scale={cfg.cfg_scale}")
            sample_kwargs["cfg_scale"] = cfg.cfg_scale
        else:
            print_rank_0("using non-cfg for sampling...")
        sample_kwargs["sample_mode"] = sample_mode
        ##############
        ...
        out_sample_global = accelerator.gather(samples.contiguous().to(device))
        model.train()
        out_sample_global = cityscapes_only_categories_indices_segmentation_to_img(
            out_sample_global
        )
        samples = cityscapes_only_categories_indices_segmentation_to_img(samples)
        return out_sample_global, samples, vis_config
    ...
    while train_steps < cfg.data.train_steps:
        ...
        if train_steps % cfg.data.sample_vis_every == 0 and train_steps > 0:
            ...
            if cfg.data.name.startswith("cs"):
                out_sample_global_random, samples, vis_config = sample_cond2img(
                    bs=cfg.data.sample_vis_n,
                    cfg=cfg,
                    _sample_size=_sample_size,
                    sample_mode="mask2img",
                )

                (out_sample_global_random_img2mask, _, _) = sample_img2cond(
                    bs=cfg.data.sample_vis_n,
                    cfg=cfg,
                    _sample_size=_sample_size,
                    sample_mode="img2mask",
                )
            else:
                out_sample_global_random, samples, vis_config = sample_cond2img(
                    bs=cfg.data.sample_vis_n,
            ...
            if accelerator.is_main_process and cfg.use_wandb:
                ...
                if cfg.data.name.startswith("cs"):
                    wandb_dict.update(
                        wandb_visual_dict(
                            "vis/sample_random_img2mask",
                            out_sample_global_random_img2mask,
                            is_video=is_video,
                        )
                    )
        ...
        if train_steps % cfg.data.sample_fid_every == 0 and train_steps > 0:
            with torch.no_grad():  # very important
                ...
                for _b_id in tqdm(
                    ...
                    out_sample_global, samples, vis_config = sample_cond2img(
        ...
        if train_steps % cfg.ckpt_every == 0 and train_steps > 0:
            # ckpt_every will not be used any more
            if accelerator.is_main_process:
                checkpoint = {
